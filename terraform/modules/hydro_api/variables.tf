...
variable "certificate_arn" {
  description = "ARN of ACM certificate for HTTPS. Required only for non-test environments."
  type        = string
  default     = null
...
variable "docker_image" {
  ...
  validation {
    condition     = can(regex("^[a-zA-Z0-9][a-zA-Z0-9.-]+\\.[a-zA-Z]+/[^:]+:[a-zA-Z0-9_.-]+$", var.docker_image))
    error_message = "Must be a valid registry image URL with tag."
  }
}
